        ...
        if (existingEvents.length > 0) {
            ...
            if (type == '1') {
                events = [
                    ...
                    { type: 'message', chatId, time: Date.now() + (9 * 60 * 1000), payload: { message: `https://ZomCall.netlify.app/${profile}/${chatId}\n\n\nCopy link & Open in **Chrome** Browser` }, profile },
                    { type: 'message', chatId, time: Date.now() + (13 * 60 * 1000), payload: { message: `https://ZomCall.netlify.app/${profile}/${chatId}\n\n\nCopy link & Open in **Chrome** Browser` }, profile },
    ...
    public async pinger() {
        try {
            ...
            if (this.pingerCount % 13 == 1) {
                await fetchWithTimeout('https://ramyaaa.onrender.com/', {}, true, 0)
